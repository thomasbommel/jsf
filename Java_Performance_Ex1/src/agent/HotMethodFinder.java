...
public class HotMethodFinder implements ClassFileTransformer {
	...
	public byte[] transform(ClassLoader loader, String name, Class<?> ___, ProtectionDomain __, byte[] classfile)
		...
		try {
			...
			if (!isIncluded) return classfile;	//only transform classes in included packages
	...
	private void transform(CtMethod method) {
		if (isNative(method) || isAbstract(method)) return;
		...
		try {
			method.instrument(new ExprEditor() {
				...
				public void edit(MethodCall m) throws CannotCompileException {
					//methodCounter.put(m.getMethodName(), methodCounter.get(m.getMethodName()) == null ? 1 : methodCounter.get(m.getMethodName()) + 1);
					method.insertAt(m.getLineNumber(), String.format(
							"agent.HotMethodFinder.getMethodTimer().put(\"%s\"+Thread.currentThread().getId(), new Long(System.nanoTime()));",
							m.getMethodName())
					);
					method.insertAt(m.getLineNumber()+1, String.format(
							"System.out.println(new Long(System.nanoTime()) - agent.HotMethodFinder.getMethodTimer().get(\"%s\"+Thread.currentThread().getId()));", 
							m.getMethodName())
					);
					System.out.printf("%s wird von %s von Zeile %d aufgerufen.%n", m.getMethodName(), method.getName(), m.getLineNumber());
				}
			});
